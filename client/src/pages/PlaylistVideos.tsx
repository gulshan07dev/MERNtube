...
export default function PlaylistVideos() {
  ...
  return (
    <Layout className="w-full flex max-lg:flex-col max-lg:gap-7 min-h-full gap-3 py-5">
      {isFetchingPlaylist ? (
        <Skeleton className="lg:w-[30%] w-full lg:min-h-full h-[500px] lg:sticky lg:top-0 rounded-lg" />
        ...
        <>
          ...
          <ScrollPagination
            ...
            loading={isFetchingVideos || isFetchingPlaylist || !playlist?._id}
            error={videosFetchingError}
          ...
          >
            {(isFetchingVideos || isFetchingPlaylist || !playlist?._id) &&
            currentPage == 1 ? (
              Array.from({ length: 10 }).map((_, idx) => (
                <div
                  key={idx}
                  className="w-full flex gap-3 p-3 rounded-lg cursor-pointer"
                >
